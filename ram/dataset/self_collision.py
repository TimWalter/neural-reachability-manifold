...
PAIR_COMBINATIONS = [torch.triu_indices(2 * dof, 2 * dof, offset=2) for dof in range(1, 12)]
...
def collision_check(mdh: Float[Tensor, "*batch dofp1 3"],
    ...
    *batch_shape, dof, _ = mdh.shape
    ...
    if not debug:
        ...
        critical_distance = torch.where(collisions, distances, torch.ones_like(distances) * torch.inf).min(
            dim=-1).values.reshape(batch_shape)
        critical_distance_end = torch.where(collisions_end, distance_end,
                                            torch.ones_like(distance_end) * torch.inf).min(dim=-1).values.reshape(
            batch_shape)
