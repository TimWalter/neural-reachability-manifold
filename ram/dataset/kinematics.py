...
from ram.dataset import r3, so3
from ram.dataset.self_collision import EPS, collision_check
...
@contextmanager
def suppress_cpp_cerr():
    """Context manager to suppress low-level C++ std::cerr output."""
...
def numerical_inverse_kinematics(inp_mdh: Float[Tensor, "dofp1 3"], inp_poses: Float[Tensor, "batch 4 4"],
    ...
    """
    Computes inverse kinematics for a robot defined by modified Denavit-Hartenberg parameters numerical via a LM solver
    with random restarts.

    Args:
        inp_mdh: Contains [alpha_i, a_i, d_i] for each joint.
        inp_poses: The desired poses (homogeneous transforms) to find joint angles for.
        num_seeds: Number of restarts.
    ...
    for batch_idx in range(0, inp_poses.shape[0], 10000):
        poses = inp_poses[batch_idx:batch_idx + 10000]
