...
def set_level(level: int = 3):
    ...
    MAX_DISTANCE_BETWEEN_CELLS = {1: 0.6527, 2: 0.3308, 3: 0.1654, 4: 0.1391}[LEVEL]
    MIN_DISTANCE_BETWEEN_CELLS = {1: 0.6283, 2: 0.3142, 3: 0.1571, 4: 0.0793}[LEVEL]
...
def to_index(orientation: Float[Tensor, "batch 3 3"]) -> Float[Tensor, "batch 3"]:
    ...
    """
    func = lambda x: Rotation.from_matrix(x, assume_valid=True).as_rotvec()
    compiled_safe_func = torch.compiler.allow_in_graph(func)
    return compiled_safe_func(orientation)
...
def from_index(rot_vec: Float[Tensor, "batch 3"]) -> Float[Tensor, "batch 3 3"]:
    ...
    Notes:
        This seems differentiable via torch.autograd, but is only since version 2.12.
    """
    func = lambda x: Rotation.from_rotvec(x).as_matrix()
    compiled_safe_func = torch.compiler.allow_in_graph(func)
    return compiled_safe_func(rot_vec)
...
@jaxtyped(typechecker=beartype)
def log(orientation1: Float[Tensor, "*batch 3 3"], orientation2: Float[Tensor, "*batch 3 3"]) \
