...
parser.add_argument("--set", type=str, default="test9", choices=["train", "val", "test"],
                    help="For which set to sample")
parser.add_argument("--dof", type=int, default=9, help="number of degrees of freedom")
...
for idx, morph in enumerate(tqdm(morphs, desc=f"Generating {args.dof} DOF robots")):
    ...
    if buffer_offset == buffer.shape[0] or idx == morphs.shape[0] - 1:
        ...
        print(f"Reachable: {active_data[:, -1].sum() / active_data.shape[0] * 100:.2f}%")
